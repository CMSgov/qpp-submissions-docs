  ...
  render() {
    return (
      <div id="provider">
        ...
        <p>A submission to the Submissions API requires a Taxpayer Identification Number (TIN) and may include a National Provider Identifier (NPI). These identifiers are used to accurately attribute the submission to a provider. A provider's profile includes information which is used to trigger special scoring scenarios.</p>
        <p><b>Note:</b> For the Beta 1 Release, no special scoring scenarios will be applied in the production environment. Every submission will be attributed with the default provider profile which incurs no special scoring.</p>
        <p>In order to support testing of special scoring scenarios, the Submissions API features a provider profile stub in the <a href="http://qpp-submissions-sandbox.navapbc.com/">sandbox</a> environment. In this environment, the Submissions API enforces the use of fake TINs, requiring TINs to start with 3 "000"s.</p>
        <p>The trailing 6 positions of fake TINs are used to populate a provider profile stub. The stub logic is detailed below and enables a tester to provide variations of fake TINs which result in different provider profiles.</p>
        <p>The provider profile stub enforces the following rules:</p>
        <ul>{profileStubItems}</ul>
      </div>
    );
  }
}
