  ...
  render() {
    return (
      <div id='provider'>
        ...
        <p className='ds-text--lead'><b>Note:</b> Currently, no special scoring scenarios are applied in the production environment. Every submission is attributed with the default provider profile which incurs no special scoring.</p>
        <p className='ds-text--lead'>In order to support testing of special scoring scenarios, the Submissions API features a provider profile stub in the <a href='http://preview.qpp.cms.gov/api/submissions/public/docs'>sandbox</a> environment, for the /score endpoint. In this environment, the Submissions API enforces the use of fake TINs, requiring TINs to start with 3 "000"s. (The /score-preview sandbox endpoint does not use the provider profile stub; instead, it uses the default provider profile, just like /score-preview in other environments.)</p>
