  ...
  render() {
    return (
        <div className="ds-u-measure--wide">
          ...
          <p className="ds-text--lead">Build direct integrations with other software to test programmatically submitting QPP performance data to CMS. Using the data we expose through the API, you can:
          <ul>
          ...
          <p>The private beta is different from the public sandbox in that it is authenticated and accepts personally identifiable information (e.g. Taxpayer Identification Numbers and National Provider Identifiers). This mirrors the general availability Submissions API, which will be released in mid-December, so that integrators can better prepare to integrate with the Submissions API for real-time usage.</p>

          <h2 className="ds-h2">Authentication</h2>
          <p className="ds-text--lead">Authenticate your account when using the API by including your secret API key in the header of every request to the API using the key value pari: "Authentication":"Bearer YOUR API TOKEN".</p>
          <p className="ds-text--lead">Learn more about requesting an API key below. Your API key carries many privileges, so be sure to keep them secret! Do not share your API keys in publicly accessible areas such as GitHub, client-side code, and so forth.</p>
          <p className="ds-text--lead">API requests without authentication will fail.</p>

          <h2 className="ds-h2">Permissions</h2>
          <p className="ds-text--lead">The Submissions API has three types of objects: submissions, measurement sets, and measurements. A submission object contains any performance data submitted on behalf of a single MIPS-eligible clinician or practice. A measurement set object represents a set of performance data related to one specific category (Quality, Improvement Activities, or Advancing Care Information), and is tied to a submission object. A measurement object represents one single data point related to a specific measure in a given category, and is tied to a measurement set object.</p>

          <h2 className="ds-h2">Creating, reading, updating and deleting data</h2>
          <p className="ds-text--lead">In general, you can read, update and delete data that you yourself have submitted, regardless of whether you are a clinician/practice or an organization. However, only a MIPS-eligible doctor or practice can create a submission object for their own performance data. Organizations, such as Qualified Registries and Qualified Clinical Data Registries, can create measurement sets and add them to any submission object. In other words, MIPS-eligible doctors and practices can make POST /submissions/ requests, but organizations can only make POST /measurement-sets/ and POST /measurements/ requests.</p>

          <h2 className="ds-h2">Scoring data</h2>
          <p className="ds-text--lead">If you are associated with a third party vendor, you can only view scores based on the data you've submitted. In the future, there will be a way for MIPS-eligible clinicians and practices to grant organizations additional permissions so that the organization can get a final score that represents all data submitted on the clinician/practice's behalf (not just based on data they've submitted).</p>

          <h2 className="ds-h2">How to request an API key</h2>
          <p className="ds-text--lead">At this time, the private beta is open only to 2017 CMS-approved Qualified Registries ("registries") and Qualified Clinical Data Registries ("QCDRs").  To request an API key, fill out <a href="https://goo.gl/forms/85W7sk8s6Sf0fOsN2">this form</a>.</p>
          <p className="ds-text--lead">We will verify that you are associated with a 2017 CMS-approved registry or QCDR, and manually generate an API key for you. Your API key permits authenticated access to the Submissions API private beta, and will expire November 1, 2017. After this date, you will be able to request a new API key through an automated, self-service process.</p>
          <p className="ds-text--lead">When you get this key:
          <ul>
          ...
          <p className="ds-text--lead">Data submitted to the Submissions API private beta will not count toward a MIPS-eligible clinician's final score in the 2017 Quality Payment Program.</p>
          <p className="ds-text--lead">Since this is a Beta release of the Submissions API, this API may be changed in backward-incompatible ways and is not subject to any SLA or deprecation policy. This API is not intended for real-time usage in critical applications.</p>
          ...
          <p className="ds-text--lead">If you have feedback about working with the private beta Submissions API, post in the <a href="https://groups.google.com/forum/#!forum/qpp-apis">Google Group for QPP APIs</a>.</p>
