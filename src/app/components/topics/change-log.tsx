...
const submissionChangesTable = [
  ['8/15/21', 'Updates for Y5 Traditional MIPS Submissions and Scoring for PY2021 are now supported.', `
    <ul>
      <li>
        Added validation that only QCDR vendors who are approved by the QCDR Measure Steward can sbmit that QCDR measure.
      </li>
      <li>
      ...
      <li>
        For PI category measurementSets, added validations so that PI can only be submitted where entityType = group, individual. (PI cannot be submitted for APM entities).
      </li>
      <li>
  ...
  ['8/15/21', 'For APM Participants, APP Submissions and Scoring for PY2021 are now supported.', `
    <ul>
      ...
      <li>
        MeasurementSets can be submitted to the APP program for entityType(s) = apm, group, individual. (Virtual Groups cannot submit to the APP program as they cannot be APM participants). 
      </li>
      <li>
  ...
  ['8/15/21', '', `
    <ul>
      ...
      <li>
        For measurementSets object, added new programName = pcf. (Note: for PCF submissions the programName can be upper or lowercase.)
      </li>
      <li>
...
const measuresChangeTable = [
  ['8/15/20', `<code>allowedPrograms</code> is added to all quality Measures.`, `For category= quality, <code>allowedPrograms</code> is added to list the programs to which the measure <i>can</i> be submitted.`],
  ['8/15/20', `<code>requiredForPrograms</code> is added to all quality Measures.`, `For category= quality, <code>requiredForPrograms</code> is added to list the programs to which the measure <i>must</i> be submitted.`],
  ['8/15/20', `<code>allowedVendors</code> is added to all QCDR Measures.`, `For QCDR Measures (<code>isRegistryMeasure = true</code>), <code>allowedVendors</code> is added to list the VendorIDs that are allowed to submit that measure.`],
  ...
  ['8/15/20', `<code>weight</code> attribute is removed for PI measures.`, 'Where "category": "pi" "weight": attribute is removed'],
];
